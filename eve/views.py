...
def feedback(request):
    userSuggestion = request.POST.get('userSuggestion', None)
    #userSuggestion = 4
    prediction = request.POST.get('prediction',None)
    isCorrect=False
    if(prediction == userSuggestion):
        isCorrect=True
    else :
        isCorrect=False
    pkid = request.POST.get('id', None)
    #pkid = "08ee82ef-2ec8-4e29-ac1c-dfbcc2b882cf"
    File.objects.filter(pk=pkid).update(userSuggestion=userSuggestion,isCorrect=isCorrect)
